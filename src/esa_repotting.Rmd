...
#reading in data
taxon.path <- read_csv(here('data', 'illumina_data', 'otu_unite_repotting.csv'))%>%  # taxon path output from UNITE 
  clean_names() %>% 
  dplyr::rename(sequence = x1)

funguilds <- read_csv(here('data','funguild_docs','otu_unite_repotting_forfunguild.guilds.csv')) %>%  # guild data output from FUNGuild
 clean_names() %>% 
  dplyr::rename(sequence = x1) %>% 
  select(!taxonomy) 
...
otu_clusters = read_csv(here("data", "sanger_data", "repotting_clusters.csv")) %>% 
...
asv.tab <- read_csv(here('data', "repotting",'repotting_otu_table.csv')) # otu table output from DADA2 and DECIPHER
asv.tab <- t(asv.tab)
...
```{r}
ggplot(sym_root, aes(x= NMDS1, y= NMDS2,color = soil_type)) + 
  geom_point(aes( shape = seedling_sp)) +
  lims(x= c(-.3, .3))+
  stat_ellipse() +
  scale_color_manual(
  ...
  scale_shape_manual(name = "Seedling Species", breaks= c("P", "Q"), labels = c("Bigcone", "Canyon Oak"), values = c(17,15))+
  theme_minimal()

ggplot(sym_soil, aes(x= NMDS1, y= NMDS2,color = soil_type)) + 
  geom_point(aes( shape = seedling_sp)) +
  lims(x= c(-.3, .3))+
  stat_ellipse() +
  scale_color_manual(
...
#ggsave(here("figures", "nmds_repotting_sym.pdf"))

ggplot(nmds_sym, aes(x= NMDS1, y= NMDS2,color = seedling_sp)) + 
  geom_point(aes( shape = soil_type)) +
  lims(x= c(-.3, .3))+
  scale_color_manual(name = "Seedling Species", breaks= c("P", "Q"), labels = c("Bigcone", "Canyon Oak"), values = c("darkseagreen4", "darkgoldenrod2")) +
  stat_ellipse()+
  scale_shape_manual(
    values = c(15, 17, 19), name = "Soil Type",
    breaks = c("P", "Q", "C"), labels = c("Bigcone", "Canyon Oak", "Control")) +
    theme_minimal()
#ggsave(here("figures", "nmds_repotting_sym_alternate.pdf"))
...
```{r}
ggplot(nmds_sym %>% 
         filter(seedling_sp =="Q"), aes(x= NMDS1, y= NMDS2,color = soil_type)) + 
  geom_point(aes()) +
  lims(x= c(-.3, .3))+
  stat_ellipse()+
  scale_color_manual(
  ...
  theme_minimal()+
  labs(title = "Canyon Oak Dissimilarity")
#ggsave(here("figures", "nmds_repotting_sym_oaks.pdf"))
ggplot(nmds_sym %>% 
         filter(seedling_sp =="P"), aes(x= NMDS1, y= NMDS2,color = soil_type)) + 
  geom_point(aes()) +
  lims(x= c(-.3, .3))+
  stat_ellipse()+
  scale_color_manual(
  ...
  theme_minimal()+
  labs(title = "Bigcone Dissimilarity")
#ggsave(here("figures", "nmds_repotting_sym_bigcone.pdf"))
```

```{r}
ggplot(nmds_sym %>% 
         filter(soil_type =="Q"), aes(x= NMDS1, y= NMDS2,color = seedling_sp)) + 
  geom_point(aes()) +
  lims(x= c(-.3, .3))+
  stat_ellipse()+
  scale_color_manual(
    values = c("darkseagreen4", "darkgoldenrod2"), name = "Seedling Species",
    breaks = c("P", "Q"), labels = c("Bigcone", "Canyon Oak")
  )+
  theme_minimal()+
  labs(title = "Oak Soil Dissimilarity")

#ggsave(here("figures", "nmds_repotting_sym_oaksoil.pdf"))

ggplot(nmds_sym %>% 
         filter(soil_type =="P"), aes(x= NMDS1, y= NMDS2,color = seedling_sp)) + 
  geom_point(aes()) +
  lims(x= c(-.3, .3))+
  stat_ellipse()+
  scale_color_manual(
  ...
  theme_minimal()+
  labs(title = "Bigcone Soil Dissimilarity")
#ggsave(here("figures", "nmds_repotting_sym_bigconesoil.pdf"))


ggplot(nmds_sym %>% 
         filter(soil_type =="C"), aes(x= NMDS1, y= NMDS2,color = seedling_sp)) + 
  geom_point(aes()) +
  lims(x= c(-.3, .3))+
  stat_ellipse()+
  scale_color_manual(
  ...
  theme_minimal()+
  labs(title = "Control Soil Dissimilarity")

#ggsave(here("figures", "nmds_repotting_sym_controlsoil.pdf"))

```


```{r}
asv.conf.sym.rs = asv.conf.sym[,1:106] %>% 
  mutate_if(is.character,as.numeric)

summat = as.matrix(asv.conf.sym.rs)/as.matrix(asv.conf.sym.rs) 
summat[is.na(summat)] <- 0
colSums(summat)

nmds_sym  = nmds_sym%>%
  mutate(richness = colSums(summat))

ggplot(nmds_sym, aes(y=richness, x=seedling_sp,fill = soil_type ))+
  geom_boxplot()


asv.conf.ect.rs = asv.conf.ect[,1:106] %>% 
  mutate_if(is.character,as.numeric)

summat = as.matrix(asv.conf.ect.rs)/as.matrix(asv.conf.ect.rs) 
summat[is.na(summat)] <- 0
colSums(summat)
...
ggplot(nmds_ect, aes(y=richness, x=seedling_sp,fill = soil_type ))+
  geom_boxplot()
...
```{r}
ect.tib = asv.conf.ect %>% 
  (t) %>% 
  as.data.frame() %>% 
  tibble() %>% 
  mutate(seedling_id = colnames(asv.conf.ect)) %>% 
  mutate(samtype = substr(seedling_id, 5,8))%>% 
  mutate(rep = substr(seedling_id, 9,10))%>% 
  mutate(seedling_id = substr(seedling_id, 1,4)) %>% 
  relocate(seedling_id, samtype, rep)  %>% 
  dplyr::filter(samtype == 'root')

colnames(ect.tib)[4:19] <- asv.conf.ect$taxon

ect.tib = ect.tib[1:106,]

ect.tib[4:19] = lapply(ect.tib[4:19], FUN = function(y){as.numeric(y)})
...
Merged_df<-as.data.frame(do.call(cbind,by(t(df),INDICES=names(df),FUN=colSums)))

ecto.combined = bind_cols(ectr[,1:3], Merged_df)
ect.long = ecto.combined%>% 
  pivot_longer(cols = 4:12, names_to = "taxon", values_to = "reads") %>% 
  group_by(seedling_id, rep) %>% 
  slice(which.max(reads)) %>% 
  filter(reads >0) %>% 
  left_join(sample_table %>% 
              filter(type == "root"))

ggplot(ect.long, aes(x = seedling_id, fill = taxon)) +
  geom_bar() +
