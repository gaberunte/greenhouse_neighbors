...
```{r, message = F}
folder = "data/sanger_data/repot_set_dec082023"
...
sample_sheet = sample_sheet_build %>% 
...
tip_seqs = metadata %>% 
  ...
  select(-project) %>% 
  left_join(sample_sheet, by = c("re_plate", "re_column", "re_row")) %>% 
...
python /Users/Gabe/Desktop/phd/FUNGuild-master/Guilds_v1.1.py -otu /Users/Gabe/Desktop/phd/ghecto/data/sanger_data/repot_set_dec082023/taxa_funguild.txt
```
...
sample_sheet_view = sample_sheet %>% 
...
ggplot(view_set, aes(x = morphotype, fill = genus_id))+
  geom_bar()+
  scale_fill_manual(breaks = c("No Amp", "Bad Seq", "Cenococcum", "Peziza",
                               "Pustularia", "Rhizopogon", "Suillus", "Tuber"),
                    values = c("azure3", "azure4", "firebrick2", "darkorange2", "gold2", "palegreen4", "steelblue", "mediumpurple"))
```
