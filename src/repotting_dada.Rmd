...
path <- "/Volumes/Gabe_Runte_EHD/ghecto_seqs/ghecto_repotting/fastqs"
```
...
```{r}
cutadapt <-  "/Users/Gabe/Library/Python/2.7/bin/cutadapt"
...
merged_seqtab <- seqtab.nochim %>%
  ...
  filter(cluster != "OTUNA")

write_csv(merged_seqtab, here("data", "repotting_otu_table.csv"))

merged_seqmat= merged_seqtab%>%
  # convert back to a matrix in the original orientation
  column_to_rownames("cluster") %>%
  as("matrix") %>%
  t

clusters_otu = clusters%>% 
  mutate(otu = paste("OTU", cluster, sep = ""))
write.csv(clusters, here("data", "repotting_clusters.csv"))
```
...
unite.taxa = assignTaxonomy(clusters$sequence, unite.ref, multithread = TRUE, tryRC = TRUE)
#taxa <- assignTaxonomy(seqtab.nochim, unite.ref, multithread = TRUE, tryRC = TRUE)
write.csv(unite.taxa, here("data", "otu_unite_repotting.csv"))
```

```{r}
write.csv(seq.tab2, here("data", "repotting_asv_table.csv"))
```