...
colonization = read_csv(here("data", "ghecto_repotting_root_pulling_data.csv")) %>% 
  dplyr::select(1:3) %>% 
...
ggplot(photo_data %>% 
         filter(seedling_sp == "P"), aes(x= pct_col,y= above_area_cm2, color = soil_type )) + 
  geom_point()+
  scale_color_manual(
    values = c("darkseagreen4", "darkgoldenrod2", "cadetblue3" ), name = "Soil Type",
    breaks = c("P", "Q", "C"), labels = c("Bigcone", "Canyon Oak", "Control"))+
...
, fill = neighb.soil)) +
  ...
  labs(x = "Soil Type",
       y= "Growth since repotting")
#ggsave(here("figures", "bigcone_growthsincerepotting.pdf"), height = 5, width = 5)
...
ggplot(master8 %>% 
    ...
    geom_boxplot(aes(x = soil_type, y = pct_col
, fill = soil_type)) +
  scale_fill_manual(
    values = c("darkseagreen4", "darkgoldenrod2", "cadetblue3" ), name = "Soil Type",
    breaks = c("Bigcone", "Canyon Oak", "Control"), labels = c("Bigcone", "Canyon Oak", "Control"))+
...
#ggsave(here("figures", "bigcone_colonization.pdf"), height = 7, width = 7)
