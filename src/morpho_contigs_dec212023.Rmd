...
```{r, message = F}
folder = "data/sanger_data/morpho_dec212023"
...
morpho = read_csv(here("data/harvest", "harvest_roottips - all.csv")) %>% 
  clean_names() %>% 
...
python /Users/Gabe/Desktop/phd/FUNGuild-master/Guilds_v1.1.py -otu /Users/Gabe/Desktop/phd/ghecto/data/sanger_data/morpho_dec212023/taxa_funguild.txt 
```
...
all_attempts  = read_csv(here("data/harvest", "harvest_tipseq_printout.csv")) %>% 
  mutate(sampleset = "m5x") %>% select(seedling_id, plate, column, row, sampleset, comb_column, comb_row) %>% 
  bind_rows(read_csv(here("data/harvest", "harvest_tipseq_printout2.csv")) %>% 
  mutate(sampleset = "m2x") %>% select(seedling_id, plate, column, row, sampleset, comb_column, comb_row)) %>% 
  bind_rows(read_csv(here("data/harvest", "harvest_tipseq_printout3.csv")) %>% 
  mutate(sampleset = "m3x") %>% select(seedling_id, plate, column, row, sampleset, comb_column, comb_row)) %>% 
  bind_rows(read_csv(here("data/harvest", "harvest_tipseq_printout4.csv")) %>% 
  mutate(sampleset = "m4x") %>% 
    rename(comb_column = newcolumn, comb_row = newrow) %>% 
    select(seedling_id, plate, column, row, sampleset, comb_column, comb_row))



all_seqs = read_csv(here(folder, "all_seqs_untrimmed.csv")) %>% 
  clean_names()%>% 
  separate(name, into = c("project", "sampleset", "comb_column", "comb_row")) %>% 
  mutate(comb_column = as.numeric(comb_column)) %>% 
  select(-project) %>% 
  left_join(all_attempts) %>% 
  left_join(morpho %>%  select(plate, column, row, morphotype))
...
ggplot(total_df %>% 
         filter(seedling_sp == "P"), aes(x = as.factor(seedling_id), fill = if_genus))+
  geom_bar()+facet_wrap(~morphotype, scales = "free")+
  scale_fill_manual(breaks = c("Melanogaster", "Pustularia", "Rhizopogon", "Tuber", "Wilcoxina", "Bad Seq", "PCR Fail"),
                     name = "Genus", values = c("#e65247","#e89923", "#f0dc5d", "#4dc44d", "#4287f5", "darkgray", "gray"))+
  theme_minimal()+ theme(axis.text.x = element_blank())
ggplot(total_df %>% 
         filter(seedling_sp == "Q"), aes(x = as.factor(seedling_id), fill = if_genus))+
  geom_bar()+facet_wrap(~morphotype, scales = "free")+
  scale_fill_manual(breaks = c("Melanogaster", "Pustularia", "Rhizopogon", "Tuber", "Wilcoxina", "Bad Seq", "PCR Fail"),
                     name = "Genus", values = c("#e65247","#e89923", "#f0dc5d", "#4dc44d", "#4287f5", "darkgray", "gray"))+
  theme_minimal()+ theme(axis.text.x = element_blank())
...
ggplot(total_df %>% 
         ...
         filter(morphotype =="D"), aes(x = as.factor(seedling_id), fill = if_genus))+
  geom_bar()+facet_grid(rows = vars(soil_type), cols = vars(neighb_st), scales = "free")+
  scale_fill_manual(breaks = c("Melanogaster", "Pustularia", "Rhizopogon", "Tuber", "Wilcoxina", "Bad Seq", "PCR Fail"),
                     name = "Genus", values = c("#e65247","#e89923", "#f0dc5d", "#4dc44d", "#4287f5", "darkgray", "gray"))+
  theme_minimal()+ theme(axis.text.x = element_blank())
